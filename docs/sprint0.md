...
Sprint 0 is the onboarding sprint. Its purpose is to get every team member up and running with the project, develop a shared picture of what the system does and how it is built, establish the team's working standards, and complete a small but real software engineering workflow from beginning to end. The skills and habits you establish in Sprint 0 carry through the rest of the semester.

!!! warning "Graduate students"
    Graduate students are held to a higher bar throughout the course. For Sprint 0, this means more thorough architecture analysis in D1, more ambitious and better-reasoned issues in D2, higher-quality standards documentation in D3, and more technically detailed PR write-ups in D5. See individual deliverable sections for specifics.
...
Every team member must have the project running locally by the end of Sprint 0.
...
4. Read the project's architecture documentation and explore the codebase enough to answer these questions:
    - What does the system do? Who are its users?
    ...
    - How do developers typically contribute? What is the PR and review workflow?
...
**Evidence (for D6 &mdash; Sprint Report):**

- Screenshot of the app running locally.
- Screenshot of the test suite passing.
- Brief written summary covering the four questions above (included in D6).
...
!!! info "Issues, not a document"
    The feature backlog lives in the project's GitHub issue tracker — not in a separate file. Create each proposed feature as a GitHub Issue with appropriate labels (see below). The sprint report links to these issues; there is no `backlog.md` to submit.
...
- **2 ambitious** features — larger improvements or new capabilities that would take a Sprint or more. Scope them thoughtfully; you will break them down later. Think about what real users of this project actually want.

There is no upper limit. If you have more ideas, open more issues.

**What makes a good issue:**
A feature is a user-visible or developer-visible change that makes the product better &mdash; a new capability, a significant improvement to an existing one, or a meaningful non-functional improvement (performance, accessibility, security). Each issue must include:

| Field | Description |
|---|---|
| **Title** | Short, descriptive, action-oriented (e.g., "Add keyboard shortcut to dismiss notification toasts") |
| **Summary** | 2&ndash;4 sentences: what the feature is, why it matters, who benefits |
| **User story** | "As a [role], I want [action] so that [outcome]" — or a short scenario |
...
**Labels to apply** (use the project's existing labels where available; create these if they don't exist):
...
| `csci435` | Marks all course-contributed issues |
...
!!! info "Aim high on the ambitious ones"
    Sprint 0 is the time to think big. Some of these features will be scoped, decomposed, or merged during Sprint 1 requirements analysis &mdash; that is expected. Propose features you would actually want to use or build.

!!! info "Graduate students"
    For each proposed issue, graduate students must also include: an assessment of which components/layers are affected, one or two key risks or unknowns, and whether the feature has community backing (link to upstream discussion if yes).

---

#### D3 &mdash; Standards & Guidelines Document

Each team writes an initial **standards and guidelines document** for how the team will work throughout the semester. This is a living document &mdash; you will refine it as you learn more, but you must commit to a first version in Sprint 0.
...
- The existing Definition of Done (look in CONTRIBUTING.md, PR templates, or the issue tracker)
...
| **Pull request process** | PR size limits; required reviewers; how to write a PR description; merge criteria |
...
You may additionally use chat-based tools (Claude.ai, ChatGPT, Gemini, Grok, DeepSeek) for non-agentic tasks such as understanding code, drafting, or research.
...
**Course Claude Team plan:** The instructor will distribute seats on a Claude Team plan for the class. This gives each team member access to Claude without the free tier's rate limits. Details and instructions will be posted on Zulip. If you cannot access the Team plan, Claude's free tier is an acceptable fallback.

**3. Set up AI log capture.** Full instructions are on the **[AI Log Instructions](ai-logs.md)** page. In brief:

- **Agentic CLI tools** (Claude Code, Gemini CLI, Codex CLI): use [SpecStory](https://specstory.com/) — `specstory run claude` wraps your session automatically.
...
- **Web chat** (Claude.ai, ChatGPT, Gemini, Grok, DeepSeek): use the browser exporter extensions listed on the [AI Log Instructions](ai-logs.md) page.
- **Claude Desktop**: not supported by SpecStory — switch to Claude Code CLI for project work.

Logs are stored in `ai-logs/sprint0/<your-github-username>/` in the team repo. See [AI Log Instructions](ai-logs.md) for naming conventions and the issue comment format.
...
**Submission:** No separate deliverable. Evidence of log setup and a usage summary are included in D6 (Sprint Report).

---

#### D5 &mdash; Pull Requests

The core hands-on activity of Sprint 0 is making real changes to the codebase and going through the full contribution workflow. Every team member opens **exactly 2 pull requests** in Sprint 0.

**How the work is assigned:**

The instructor will post **2&ndash;3 issues** in your project's issue tracker during the first week of Sprint 0. These are small, clearly scoped changes &mdash; worked examples of the contribution workflow on your specific codebase.

- 2&ndash;3 team members (coordinate as a team) will implement the instructor-assigned issues.
- Every other team member picks 2 issues from the team's feature backlog (D2) or the project's existing issue tracker &mdash; issues at small or medium scope.
...
Coordinate early &mdash; decide who implements which issues before anyone starts coding.

**Before you write any code:**

1. **Read the issue carefully.** What exactly is being asked? Ask the author on the issue if anything is unclear.
2. **Find the relevant code yourself first** &mdash; without AI. Read the directory structure, search the codebase, follow the call stack. Take notes on what you find.
...
- The PR must pass CI if CI is configured in the project. If CI is not yet configured, note this in the PR description.

!!! warning "You must be able to explain every line"
    You may use AI tools to help write code. However, you are responsible for understanding every line in your PR. Reviewers and the instructor may ask you to explain any part of your change at any time &mdash; including after the sprint. If you cannot explain code in your own PR, it will not count toward your grade regardless of whether it was merged.

!!! info "Graduate students"
    Graduate students must additionally document in the PR description: which components were affected and why, any design alternatives considered, and a brief assessment of test coverage adequacy.
...
**Accepted formats:** Markdown pushed to the repository, Google Docs (share the link in the repo), PDF, or Word document.
...
| **Project overview** | What the system does; key users; main components and technologies; codebase organization. Answer the four questions from D1. (~300&ndash;500 words) |
| **Feature backlog summary** | How many issues were created; themes or categories; top 3&ndash;5 features the team is most interested in pursuing. Link to the issues (filtered by `csci435` label). |
...
| **Sprint 1 plan** | Which features the team intends to tackle; high-level work breakdown; any setup still needed |

**Length:** No minimum or maximum. Be specific and concrete &mdash; vague bullet-point lists are not useful evidence.
...
4. Write a brief release description: what the team set up, what was changed, and what is planned for Sprint 1.
...
Each team member completes the Sprint 0 reflection survey individually. The link will be distributed via Blackboard and Zulip.
...
|---|---|---|
| D1 &mdash; Project Setup | 10 | Evidence: screenshots + written project overview in sprint report |
...
| D5 &mdash; Pull Requests | 30 | Per member: 15 pts each PR (max 2). Criteria: code quality, test coverage, PR description, code review, CI passing |
...
| Code was reviewed by a teammate (review comment evidence required) | 2 |
| CI passes — or a documented explanation if CI is not configured | 1 |
...
- [ ] `docs/sprint0/report.md` (or `.pdf`, `.docx`, or Google Doc link) — sprint report (D6)
...
Deliver in class **Sep 22**:

- [ ] Lightning talk (requirements posted separately)
